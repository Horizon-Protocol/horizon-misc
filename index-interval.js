...
const manageSecondaryPerps = async () => {
    ...
    // const UPPER_SAFE_UPTIME = 300_000; Assuming Keeper is restarted after this point)
                        ...
                        if (mainKeeperUptime > LOWER_SAFE_UPTIME && mainKeeperUptime < UPPER_SAFE_UPTIME) {
                            if (secondKeeper?.pm2_env?.status == 'online') {
                                pm2.stop(SECONDARY_KEEPER, function (err, app) {
                                    ...
                                    console.log(`Primary Keeper is online for ${LOWER_SAFE_UPTIME / 1000} after a recent restart. Stopping secondary keeper at ${current.toUTCString()}`);
                                    return pm2.disconnect();
                                })
                            }
                            else {
                                const timeElapsed = Date.now();
                                const current = new Date(timeElapsed);
                                console.log(`Primary Keeper is running in safe-intervals ${LOWER_SAFE_UPTIME / 1000}/${UPPER_SAFE_UPTIME / 1000} -  No action required at ${current.toUTCString()}`);
                                return pm2.disconnect();
                            }
                        }
                        else {
                            if (secondKeeper?.pm2_env?.status != 'online') {
                                ...
                                const timeElapsed = Date.now();
                                const current = new Date(timeElapsed);
                                console.log(`Primary Keeper not in safe intervals ${LOWER_SAFE_UPTIME / 1000}/${UPPER_SAFE_UPTIME / 1000} but Secondary Keeper is already running as a backup - No action required at ${current.toUTCString()}`);
                                return pm2.disconnect();
                            }
                        }
                    }
